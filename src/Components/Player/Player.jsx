...
export default function Player() {
    ...
    useFrame(() => {
        if (playerRef.current) {
            playerRef.current.rotation.y += speed * 2;
            ...
            if (movementState.moveBackward) {
                const backwardVector = new THREE.Vector3(cameraDirection.x, 0, cameraDirection.z).normalize();
                moveVector.add(backwardVector.multiplyScalar(-speed));
            }
            
            // Apply movement to camera 
            if (moveVector.x !== 0) camera.position.x += moveVector.x;
            if (moveVector.y !== 0) camera.position.y += moveVector.y;
            if (moveVector.z !== 0) camera.position.z += moveVector.z;
